...
for (int i=0; i<a.length; i++) // Use the Knuth random perm. algorithm exchange(a, i, rand.nextInt(a.length-i)+i);
}
...
for (int i=0; i < n - offset; i += offset) // compare-exchange up compareRegions(a,i,i+offset,offset,rand);
for (int i=n-offset; i >= offset; i -= offset) // compare-exchange down compareRegions(a,i-offset,i,offset,rand);
for (int i=0; i < n-3*offset; i += offset) // compare 3 hops up compareRegions(a,i,i+3*offset,offset,rand);
for (int i=0; i < n-2*offset; i += offset) // compare 2 hops up compareRegions(a,i,i+2*offset,offset,rand);
for (int i=0; i < n; i += 2*offset) // compare odd-even regions
compareRegions(a,i,i+offset,offset,rand);
for (int i=offset; i < n-offset; i += 2*offset) // compare even-odd regions compareRegions(a,i,i+offset,offset,rand);
}
}
}
